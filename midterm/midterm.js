function drawThingy(p5, x, y) {
    //implementing Coords
    // let centerCoords = [p5.width * .5, p5.height * .5];
    let centerCoords = [x,y];
    let circleSize = p5.width * .8;
    const outerWidthEdge = .4
    let leftEdge = centerCoords[0] - (p5.width * (outerWidthEdge/2))
    let rightEdge = centerCoords[0] + (p5.width * (outerWidthEdge/2))
    ...
    const outerWidth=p5.width * outerWidthEdge

    let dialRotateOuter = outerWidth * (1 - (dialHeight / 15))
    ...
    p5.rect(leftEdge, centerCoords[1],
        outerWidth, amountDistance - centerCoords[0]
    )
    ...
    // ~~draw dial~~
    // p5.stroke((p5.mouseX) / 3, 100 + (dialHeight * 23), 200 - (p5.mouseX / 4))
    let noiseColor=p5.noise(0.05 * p5.frameCount)*255
    p5.stroke((p5.mouseX) / 3, 100 + (dialHeight * 23), noiseColor)
    p5.arc(centerCoords[0], centerCoords[1],
        p5.width * .35,
        dialRotateDial,
        stemArcS,
...
/*
const foo={name:"emma",age:20,message:(name)=>{alert("heyyy, "+name)}}
const bar=["emma",12]
...
foo.message(prompt("whats ur name"))
